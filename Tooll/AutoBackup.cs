...
namespace Framefield.Tooll
{
    public class AutoBackup : IDisposable
    {
        ...
        private static FileInfo GetLatestsArchiveFile()
        {
            ...
            var backupDirectory = new DirectoryInfo(BACKUP_DIRECTORY);
        ...
        public static void ReduceNumberOfBackups(int backupDensity=3)
        {
            ...
            foreach (var filename in Directory.GetFiles(BACKUP_DIRECTORY))
            {
            ...
            for (var i = highestIndex - 1; i >= 0; i--)
            {
                ...
                else
                {
                    if (!backupFilepathsByIndex.ContainsKey(i))
                        continue;
                    Logger.Debug("removing... old backup {0} (level 2^{2})...", backupFilepathsByIndex[i], i, b);
        ...
        const string OPERATORS_DIRECTORY = "Operators";
